...
Using the bagging method to build the model yields a Mean of squared residuals of 217089818 with the formula Cases ~ . - State - Deaths - PercentDeath. The % of variance explained by the model is only 43.5%.  The two most important variables in the model are LkdDuration and TotalPop. Using the test data we calculate a Test MSE = `r MSE.bag` with a square root of `r sqrt(MSE.bag)`. 
